...
String sql = "select *,(select delete_id from member where member.id="+code+".id) as delete_id from "+code+" where gongji !='1' order by fid desc, thread asc limit 0,4"; 
...
String sql_gongji = "select *,(select delete_id from member where member.id="+code+".id) as delete_id from "+code+" where gongji ='1' order by fid desc, thread asc limit 0,4";
	...
	<tr>
		<td style="vertical-align: top">
			<table width=500 align="center">
				<tr>
					<td class="title_font">자주 묻는 질문</td>
				</tr>
			</table>
			<table width=500>
				<tr>
					<td style="text-align: left" class = "count_font">총 게시글 수 <%=total_record_gongji %> 
					<td colspan = 4 style="text-align: right"><a href = "list_gongji.jsp?code=<%=code%>&gongji=1">더보기=></a></td>
				</tr>
			</table>
						...
						<table border=0>
							<tr>
								...
								<td style="text-align: left" >
									<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&gongji=1" style="color:red"><%=subject %><span style="color: blue">[<%=com_count %>]</span></a></b>
								</td>
							</tr>
						</table>			
					</td>
					<td width = 70 onclick="view_layer(<%=uid %>)" style="text-align: center;cursor: pointer;"  class="td_comment">
						<%=name%>
		...
		<td style="vertical-align: top">	
			<table width=500 align="center">
				<tr>
					<td class="title_font">Q&A</td>
				</tr>
			</table>
			<table width=500>
				<tr>
					<td style="text-align: left" class = "count_font">총 게시글 수 <%=total_record %> 
					<td colspan = 4 style="text-align: right"><a href = "list_normal.jsp?code=<%=code%>&gongji=2">더보기=></a></td>
				</tr>
			</table>
						...
						<table>
							<tr>
								...
								<td style="text-align: left" >
									...
									%>			<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&gongji=3"><%=subject %><span style="color: blue">[<%=com_count %>]</span></a>	
											<%}else if(session_id != null && session_id.equals(id)){%>
												<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&gongji=3""><%=subject %><span style="color: blue">[<%=com_count %>]</span></a>	
											<%}else if(session_id != null && session_id.equals(origin_id)){ %>	
												<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&gongji=3""><%=subject %><span style="color: blue">[<%=com_count %>]</span></a>
											<%}else{ %>
												<%=subject %>
											<%
									...
									%>	
									<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&gongji=2"><%=subject %><span style="color: blue">[<%=com_count %>]</span></a>
									<%} %>
...
<table width=1100 align="center" border=0>
	<tr>
		...
		<td style="text-align: right">
			...
			<a href="write_main.jsp?code=<%=code%>" style="cursor: pointer">[글쓰기]</a>
			<%}else{ %>
