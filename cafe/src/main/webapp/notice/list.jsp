...
String sql = "select *,(select member.delete_id from member where member.id="+code+".id) as delete_id from "+code+" where gongji !='1' order by fid desc, thread asc limit "+first+","+num_per_page+""; 

if(request.getParameter("search") != null){
	sql = "select *,(select delete_id from member where id="+code+".id) as delete_id from "+code+" where gongji !='1' and "+field+" like '%"+search+"%' order by fid desc, thread asc limit "+first+","+num_per_page+""; 
}
	...
	<tr>
		<td style="vertical-align: top">
			...
			<table width=1100 border=0>
				<tr>
					<td colspan=2 class="title_font">자유 게시판 </td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td width = 70 onclick="view_layer(<%=uid %>)" style="text-align: center;cursor: pointer;">
						...
						<div onmouseleave="view_layer_out(<%=uid %>)" id="lay_div[<%=uid %>]" class="click_option">
							<div style="height:30px" class="div_option"><a href="" onclick="window.open('/message/write.jsp?d_id=<%=id%>','a','width=1000,height=500')" style="color:white;">쪽지보내기</a></div>
							<div class="div_option"><a href ="" onclick="window.open('/member/mem_info.jsp?id=<%=id%>&name=<%=name%>','a','width=1000,height=500')" style="color:white;">회원정보보기</a></div>
						</div>
					</td>
							...
							<tr>
								...
								<td style="text-align: left" >
									...
									%>
										...
										<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&gongji=<%=gongji%>"><%=subject %><span style="color: blue">[<%=com_count %>]</span></a>
									<%} %>
